...
class UsersController extends AppController {
	...
	public function lost_password() {
		if(!empty($this->data)) {
			...
			if(!empty($email_exists)) {
				...
				$this->Email->from = 'sam@bravegamer.com';
                $this->Email->to = $email_exists['User']['email'];
                $this->Email->subject = 'askpesit password recovery.';
	...
	public function top(){
		$topFive=$this->User->find('all',
		array('order'=>array('User.reputation'=>'desc'),
        'limit'=>5,
        'fields'=>array('User.username','User.reputation','User.image','User.public_key')
        ));
        $this->set('users',$topFive);
	}
