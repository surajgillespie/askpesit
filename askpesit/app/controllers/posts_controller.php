...
class PostsController extends AppController {
	...
	var $paginate = array(
				'contain' => array('User', 'Tag.tag', 'Answer' => array('fields' => array('Answer.id'))),
				'limit' => 5,
				'fields' => array('Post.title', 'Post.views', 'Post.url_title','Post.CSIS','Post.ECTE','Post.ELECT','Post.MECH',
	...
	public function subjects()
	{
		...
		$this->paginate['conditions'][]['Post.type'] = 'question';
		$this->paginate['conditions'][]['Post.flags <'] = $flag_check['Setting']['value'];
		
			
		if(isset($this->passedArgs['Search.type']))
		{
			switch($this->passedArgs['Search.type']){
				case'popular':	$this->paginate['order'][]['Post.Views'] = 'DESC'; 
								$this->data['Search']['type']= $this->passedArgs['Search.type'];break;
				case'recent':	$this->paginate['order'][]['Post.timestamp'] = 'DESC'; 
								$this->data['Search']['type']= $this->passedArgs['Search.type'];break;
				default:		$this->paginate['order'][]['Post.timestamp'] = 'DESC'; 
								$this->data['Search']['type']= $this->passedArgs['Search.type'];break;
			}			
		}else{
								$this->paginate['order'][]['Post.timestamp'] = 'DESC'; 
								$this->data['Search']['type']= 'recent';
		}
		
	
		if(isset($this->passedArgs['Search.status']))
		{
			switch($this->passedArgs['Search.status']){
				case'unsolved':	$this->paginate['conditions'][]['Post.status'] = 'open'; 
								$this->data['Search']['status']= $this->passedArgs['Search.status'];break;
				case'solved':	$this->paginate['conditions'][]['Post.status'] = 'closed'; 
								$this->data['Search']['status']= $this->passedArgs['Search.status'];break;
				case'both':		$this->data['Search']['status']= $this->passedArgs['Search.status'];break;
				default:		$this->data['Search']['status']= $this->passedArgs['Search.status'];break;
			}			
		}else{
								$this->data['Search']['status']= 'both';
		}
		
		
		if(isset($this->passedArgs['Search.andor'])) {
				$andor=$this->passedArgs['Search.andor'];	
				$this->data['Search']['andor']= $this->passedArgs['Search.andor'];
		}else{
				$andor='OR';
				$this->data['Search']['andor']= 'OR';
			}
			
			
		if(isset($this->passedArgs['Search.keywords'])) {
			$key = $this->passedArgs['Search.keywords'];
	        $this->paginate['conditions'][] = array('OR' => array('Post.title LIKE' => "%$key%",'Post.content LIKE' => "%$key%"));			
	        $this->data['Search']['keywords'] = $this->passedArgs['Search.keywords'];
	        }
	   
			
			if(isset($this->passedArgs['Search.CSIS'])) {
				$this->paginate['conditions'][$andor]['Post.CSIS'] = $this->passedArgs['Search.CSIS'];
				$this->data['Search']['CSIS'] = $this->passedArgs['Search.CSIS'];
			}
		
			if(isset($this->passedArgs['Search.ECTE'])) {
				$this->paginate['conditions'][$andor]['Post.ECTE'] = $this->passedArgs['Search.ECTE'];
				$this->data['Search']['ECTE'] = $this->passedArgs['Search.ECTE'];
			}
			
			if(isset($this->passedArgs['Search.MECH'])) {
				$this->paginate['conditions'][$andor]['Post.MECH'] = $this->passedArgs['Search.MECH'];
				$this->data['Search']['MECH'] = $this->passedArgs['Search.MECH'];
			}
			
			if(isset($this->passedArgs['Search.ELECT'])) {
				$this->paginate['conditions'][$andor]['Post.ELECT'] = $this->passedArgs['Search.ELECT'];
				$this->data['Search']['ELECT'] = $this->passedArgs['Search.ELECT'];
			}
			
			if(isset($this->passedArgs['Search.BIOT'])) {
				$this->paginate['conditions'][$andor]['Post.BIOT'] = $this->passedArgs['Search.BIOT'];
				$this->data['Search']['BIOT'] = $this->passedArgs['Search.BIOT'];
			}
			
			if(isset($this->passedArgs['Search.SANDH'])) {
				$this->paginate['conditions'][$andor]['Post.SANDH'] = $this->passedArgs['Search.SANDH'];
				$this->data['Search']['SANDH'] = $this->passedArgs['Search.SANDH'];
			}
			
			if(isset($this->passedArgs['Search.MATH'])) {
				$this->paginate['conditions'][$andor]['Post.MATH'] = $this->passedArgs['Search.MATH'];
				$this->data['Search']['MATH'] = $this->passedArgs['Search.MATH'];
			}
			
			if(isset($this->passedArgs['Search.PESIT'])) {
				$this->paginate['conditions'][$andor]['Post.PESIT'] = $this->passedArgs['Search.PESIT'];
				$this->data['Search']['PESIT'] = $this->passedArgs['Search.PESIT'];
			}
			
			if(isset($this->passedArgs['Search.PESITEVENTS'])) {
				$this->paginate['conditions'][$andor]['Post.PESITEVENTS'] = $this->passedArgs['Search.PESITEVENTS'];
				$this->data['Search']['PESITEVENTS'] = $this->passedArgs['Search.PESITEVENTS'];
			}
		...
		$this->Session->setFlash('SELECT ONE', 'error');
		}
	}
	...
	public function markCorrect($public_key) {
        ...
        if($question['Post']['user_id'] != $rep['User']['id']){
        $user = array(
            'User' => array(
                'id' => $rep['User']['id'],
