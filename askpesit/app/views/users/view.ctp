...
<div id="userAvatar">
	<div id="image">
		<? if(empty($user['User']['image'])) { ?>
			<img src="/askpesit/app/webroot/img/answerAvatar.png" />
		<? }else { 
...
<div id="tabs" style="margin-top: 0px;">
		...
		<table>
			...
			<tr>
			    ...
			    <? if(($recent[$key]['History']['type'] == 'asked') || ($recent[$key]['History']['type'] == 'edited')) : ?>
			    	<a href="/askpesit/questions/<?= $recent[$key]['Post']['public_key'] ?>/<?= $recent[$key]['Post']['url_title'] ?>">
			            <!-- <?= $recent[$key]['Post']['title'] ?>-->
					</a>
			  	<? elseif($recent[$key]['History']['type'] == 'commented') : ?>
						<? if(isset($recent[$key]['Pad'])) : ?>
			        	<a href="/askpesit/questions/<?= $recent[$key]['Pad']['Post']['public_key'] ?>/<?= $recent[$key]['Pad']['Post']['url_title'] ?>">
				        	<?= $recent[$key]['Pad']['Post']['title'] ?>
						</a>
						<? elseif(isset($recent[$key]['Real'])) : ?>
						<a href="/askpesit/questions/<?= $recent[$key]['Real']['Post']['public_key'] ?>/<?=$recent[$key]['Real']['Post']['url_title']?>">
	...
	<div class="tabPanel" id="tab-3">
	    ...
	    <? if($recent[$key]['History']['type'] == 'asked') {
	        echo '<a href="/askpesit/questions/' . $recent[$key]['Post']['public_key'] . '/' . $recent[$key]['Post']['url_title'] . '">
	...
	<div class="tabPanel" id="tab-4">
		...
		<? endif; ?>
	    <? if($recent[$key]['History']['type'] == 'answered') {
	        echo '<a href="/askpesit/questions/' . $recent[$key]['Pad']['Post']['public_key'] . '/' . $recent[$key]['Pad']['Post']['url_title'] . '">
